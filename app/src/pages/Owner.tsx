...
export function InsightsPage() {
  ...
  const plusTotal = s.closings.filter((c) => c.difference > 0).reduce((t, c) => t + c.difference, 0);
  const minusTotal = minusDays.reduce((t, c) => t + c.difference, 0);
  return (
    <>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <Card title={`Where things stand on ${fmtShort(range.to)}`}>
              <div className="tiles">
                ...
                <div className="tile"><span>Distributors owe us (posting differences)</span><b className={`num ${s.distributor_diff_pending > 0 ? 'danger' : ''}`}>{num(s.distributor_diff_pending)}</b></div>
                <div className="tile"><span>Invoices not posted in POS</span><b className={`num ${s.unposted_invoices > 0 ? 'danger' : ''}`}>{s.unposted_invoices}</b></div>
                <div className="tile"><span>Purchases paid in this period</span><b className="num">{num(s.purchases_paid)}</b></div>
              </div>
            </Card>
            <Card title="Extra cash above the POS sale · closings in this period">
              {s.closings.length === 0 ? <Empty>No closings yet</Empty> : <>
                <div className="grid grid-3" style={{ marginBottom: 10 }}>
                  <div className="kpi ok"><div className="label">Total plus (extra cash)</div><div className="value num ok" data-testid="plus-total">{num(plusTotal, true)}</div><div className="hint">{s.closings.length - minusDays.length} days · avg {num(Math.round(avg), true)} / closing</div></div>
                  <div className={`kpi ${minusDays.length ? 'danger' : ''}`}><div className="label">Total minus (short)</div><div className={`value num ${minusDays.length ? 'danger' : ''}`}>{num(minusTotal)}</div><div className="hint">{minusDays.length} day{minusDays.length === 1 ? '' : 's'}</div></div>
                  <div className="kpi"><div className="label">Net over the period</div><div className={`value num ${plusTotal + minusTotal < 0 ? 'danger' : 'ok'}`}>{num(plusTotal + minusTotal, true)}</div><div className="hint">counted cash − expected, all closings</div></div>
                </div>
  ...
  const pdf = async () => {
    ...
    try {
      await exportReportPdf({
        ...
        sections: [
          { title: 'Summary', rows: [['Sales (all methods)', num(s.pos_total)], ['Purchases received', num(s.purchases_received)], ['Purchases paid', num(s.purchases_paid)], ['Expenses', num(s.expenses)], ['Owed to distributors', num(s.owed_to_distributors)], ['Distributors owe us (posting differences)', num(s.distributor_diff_pending)], ['Invoices not posted in POS', String(s.unposted_invoices)], ['Non-cash received', num(nonCashReceived)], ['Cash difference (closings)', num(s.closings.reduce((t, c) => t + c.difference, 0), true)]] },
          { title: 'Daily closing · system vs drawer', head: ['Date', 'Expected', 'Counted', 'Difference', 'Closed by', 'Status', 'Notes counted'], rows: [...s.closings.map((c) => [fmtShort(c.day), num(c.expected), num(c.counted), num(c.difference, true), c.closed_by, c.status, denominationsText(c.denominations)]), ['Total', num(s.closings.reduce((t, c) => t + c.expected, 0)), num(s.closings.reduce((t, c) => t + c.counted, 0)), num(s.closings.reduce((t, c) => t + c.difference, 0), true), '', '', '']] },
