...
export default function Today() {
  ...
  const day = today();
  const [data, setData] = useState<{ book: api.CashBook; sale: api.DailySale | null; closing: api.Closing | null; bday: api.BusinessDay | null; pending: number; unposted: number; waw: number; entries: Entry[] } | null>(null);
  useEffect(() => {
    let alive = true;
    (async () => {
      const [book, sale, closing, bday, balances, unposted, waw, payments, expenses, invoices, wawRows, profiles] = await Promise.all([
      ...
      if (alive) setData({ book, sale, closing, bday, pending: balances.reduce((s, d) => s + d.pending, 0), unposted: unposted.length, waw, entries });
    ...
    <>
      <TopBar title="Today" sub={fmtDay(day)} right={<>
        ...
        <span className="desktop-only actions">
          {profile.role !== 'cashier' && <Link className="btn" to="/sales/new">+ Sale</Link>}
          <Link className="btn" to="/purchases/new">+ Purchase</Link>
          <Link className="btn" to="/pay">+ Payment</Link>
          <Link className="btn primary" to="/expenses/new">+ Expense</Link>
        </span>
      </>} />
      <div className="content">
        <div className="grid grid-4">
          <KPI label="POS system sale" value={sale ? <Money v={sale.pos_total} /> : '—'} hint={sale ? `Cash ${num(posCash)} · Card/online ${num(sale.pos_total - sale.credit_total - (posCash ?? 0))} · Credit ${num(sale.credit_total)}` : 'Not recorded yet'} />
